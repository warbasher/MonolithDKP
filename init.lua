...
-- Reduced both calls (here and in comms.lua) to a single function
function MonDKP:UpdateLootTable(lootTable)
	-- This runs initially to default out the loot table
	--core.LootTable = core.LootTable or {}
	...
	for i=1, #lootTable do
		...
		item:ContinueOnItemLoad(function()
			...
			table.insert(lootList, {icon=icon, link=item:GetItemLink(), boss = lootTable.boss or "None"})
		end)
	end

	MonDKP:LootTable_Set(lootList)

	-- This won't work unless everyone has this version..
	--[[local lootList = {}
	for i=1, #lootTable do
		local boss = lootTable[i].boss or "None"
		local item = Item:CreateFromItemLink(lootTable[i])
		item:ContinueOnItemLoad(function()
			local icon = item:GetItemIcon()

			if (not lootList[boss]) then
				lootList[boss] = {}
			end

			table.insert(lootList[boss], {icon = icon, link = item:GetItemLink()})
		end)
	end

	for boss, lootTab in pairs(lootList) do
		for _, itemTab in lootTab do
			local addItem = true
			for _, corItemTab in core.LootTable do
				if (itemTab.link == corItemTab.link) then
					addItem = false
					break
				end
			end
			...
			if addItem then
				if (not core.LootTable[boss]) then
					core.LootTable[boss] = {}
				end

				table.insert(lootList[boss], itemTab)
			end
		end
	end

	MonDKP:LootTable_Set(core.LootTable)]]
end
...
function MonDKP_OnEvent(self, event, arg1, ...)
	...
	elseif event == "UPDATE_INSTANCE_INFO" then
		...
		for k,v in pairs(MonDKP_DB.Lockouts) do
			...
			elseif k == "Five" then
				raidString = "Onyxia"
			elseif k == "Seven" then
				raidString = "MC, BWL, AQ40, KARA, GRUUL, MAG"
			end
	...
	elseif event == "COMBAT_LOG_EVENT_UNFILTERED" then 		-- logs last 15 NPCs killed while in raid
		if IsInRaid() then 					-- only processes combat log events if in raid
			...
			if arg1 == "UNIT_DIED" and (not strfind(arg2, "Player")) and (not strfind(arg2, "Pet-")) then
				...
				-- Test code for auto dkp, seems to work well
				--[[local bossName = arg3
				print(bossName)
				if MonDKP:Table_Search(core.EncounterList, arg3) then
					MonDKP_DB.bossargs.LastKilledBoss = bossName
					MonDKP:AutoAward(1, MonDKP_DB.DKPBonus.BossKillBonus, MonDKP_DB.bossargs.CurrentRaidZone..": "..MonDKP_DB.bossargs.LastKilledBoss)
				end]]
	...
	elseif event == "LOOT_OPENED" then
		...
		if core.IsOfficer then
			...
			MonDKP:UpdateLootTable(lootTable)
		end
	end
end
...
function MonDKP:OnInitialize(event, name)		-- This is the FIRST function to run on load triggered registered events at bottom of file
    ...
    if(event == "ADDON_LOADED") then
    	...
    	if not MonDKP_DB.MinOSBid then MonDKP_DB.MinOSBid = 35 end
		if not MonDKP_DB.bossargs then MonDKP_DB.bossargs = { CurrentRaidZone = "Karazhan", LastKilledBoss = "Attunmen the Huntsman" } end
