...
local function AddGuildToDKPTable(rank)
	...
	local yearsOffline, monthsOffline = 0, 0
	local numPlayers = 0;
	local FlagRecovery = false
	local curTime = time()

	for i=1, guildSize do
		name,rankName,rankIndex,level,_,_,_,_,online,_,class = GetGuildRosterInfo(i)
		name = strsub(name, 1, string.find(name, "-")-1)			-- required to remove server name from player (can remove in classic if this is not an issue)
		local search = MonDKP:Table_Search(MonDKP_DKPTable, name)

		-- For some reason GetGuildRosterLastOnline errors out if you are online
		if (not online) then
			yearsOffline, monthsOffline = GetGuildRosterLastOnline(i)
		else
			yearsOffline, monthsOffline = 0, 0
		end

		-- safety check
		if (not yearsOffline) or (not monthsOffline) then
			yearsOffline, monthsOffline = 0, 0
		end

		-- New check: requires you to be at least 58 to be imported, and online within the last 2 months
		local passesCheck = (level >= 58) and (yearsOffline == 0) and (monthsOffline <= 1)
		if (not search) and (rankIndex == rank) and passesCheck then
			tinsert(MonDKP_DKPTable, {
				player=name,
				class=class,
				dkp=0,
...
function MonDKP:ManageEntries()
	...
	MonDKP.ConfigTab3.GuildRankDropDown = LibDD:Create_UIDropDownMenu("MonDKPConfigReasonDropDown", MonDKP.ConfigTab3)
	...
	LibDD:UIDropDownMenu_SetWidth(MonDKP.ConfigTab3.GuildRankDropDown, 105)
	LibDD:UIDropDownMenu_SetText(MonDKP.ConfigTab3.GuildRankDropDown, "Select Rank")

	-- Create and bind the initialization function to the dropdown menu
	LibDD:UIDropDownMenu_Initialize(MonDKP.ConfigTab3.GuildRankDropDown, function(self, level, menuList)
	...
	function MonDKP.ConfigTab3.GuildRankDropDown:SetValue(arg1, arg2)
		if curRank ~= arg1 then
			curRank = arg1
			curIndex = arg2
			LibDD:UIDropDownMenu_SetText(MonDKP.ConfigTab3.GuildRankDropDown, arg1)
		else
			curRank = nil
			curIndex = nil
			LibDD:UIDropDownMenu_SetText(MonDKP.ConfigTab3.GuildRankDropDown, L["SELECTRANK"])
		end
