...
function MonDKP:ClassGraph()
	local graph = CreateFrame("Frame", "MonDKPClassIcons", MonDKP.ConfigTab1, "BackdropTemplate")
	...
	for i=1, 9 do
		...
		if i==1 then
	  		graph.icons[i]:SetPoint("BOTTOMLEFT", graph, "BOTTOMLEFT", 50, 40);
		else
			graph.icons[i]:SetPoint("LEFT", graph.icons[i-1], "RIGHT", 15, 0);
		end
  		graph.icons[i]:SetColorTexture(0, 0, 0, 1)
  		graph.icons[i]:SetSize(28, 28);
  		graph.icons[i].bar = CreateFrame("Frame", "MonDKP"..i.."Graph", graph, "BackdropTemplate")
	...
	-- Druid, hunter, mage, paladin, rogue, warlock, warrior, shaman
	...
	graph.icons[9]:SetTexCoord(0.25, 0.5, 0.25, 0.5)
