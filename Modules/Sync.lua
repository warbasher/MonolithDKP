...
local function MonDKP_BroadcastFull_Status_Create()
	local f = CreateFrame("Frame", "MonDKP_FullBroadcastStatus", UIParent, "BackdropTemplate,ShadowOverlaySmallTemplate");
	...
	f.cancelButton = MonDKP:CreateButton("CENTER", f, "CENTER", 0, 10, "Cancel")
	f.cancelButton:SetSize(90,25)
	f.cancelButton:SetScript("OnClick", function()
		--[[core.BroadcastProgress.status:SetScript("OnUpdate", nil)
		MonDKP.Sync:SendData("MonDKPAllTabs", {"ToIgnore" = player}, player)
		f:Hide()]]

		local selected = "Are you sure you want to cancel? \n\n|cffDF0000Doing this will force a UI reload.\nThese reloads are faster when you delete _classic_/cache periodically."
		StaticPopupDialogs["CANCEL_BROADCAST"] = {
			text = selected,
			button1 = L["YES"],
			button2 = L["NO"],
			OnAccept = function()
		...
		StaticPopup_Show("CANCEL_BROADCAST")

	end)

	f.status = CreateFrame("StatusBar", nil, f, "BackdropTemplate")
	...
	f.status.border = CreateFrame("Frame", nil, f.status, "BackdropTemplate");
...
function MonDKP_BroadcastFull_Init()
	...
	core.Broadcast.confirmButton:SetScript("OnClick", function()
		...
		if core.Broadcast.fullCheckbox:GetChecked() == true then
			--[[local tempHistory = MonDKP_DKPHistory
			local thirtyDaysAgo = time() - 2592000
			local dkpHistory = {}
			for _, tab in pairs(tempHistory) do
				if (type(tab) == "table") and tab.date and (tab.date >= thirtyDaysAgo) then
					print("Less than 30")
					print(tab.players)
				end
			end]]
		...
		elseif core.Broadcast.guildCheckbox:GetChecked() == true then
			if core.Broadcast.mergeCheckbox:GetChecked() == true then
				--MonDKP.Sync:SendData("MonDKPMerge", tempTable, nil, UnitName("player"))
			...
			elseif core.Broadcast.fullCheckbox:GetChecked() == true then
				StaticPopupDialogs["FULL_TABS_ALERT"] = {
					...
					button2 = L["NO"],
					OnAccept = function()
						--MonDKP.Sync:SendData("MonDKPAllTabs", tempTable, player, UnitName("player"))
						MonDKP.Sync:SendData("MonDKPAllTabs", tempTable, player)
						core.Broadcast:Hide()
						MonDKP_DKPHistory.seed = 0
						MonDKP_Loot.seed = 0
...
function MonDKP_BroadcastFull_Create()
	local f = CreateFrame("Frame", "MonDKP_FullBroadcastWindow", UIParent, "BackdropTemplate,ShadowOverlaySmallTemplate");
	...
	-- Close Button
	f.closeContainer = CreateFrame("Frame", "MonDKPTitle", f, "BackdropTemplate")
...
function MonDKP_SyncDeleted()
	...
	for k,v in pairs(MonDKP_Archive) do
		if (k and (k ~= "")) and MonDKP_Archive[k] then
			-- Don't want to select the DKP meta, whatever the hell that thing is
