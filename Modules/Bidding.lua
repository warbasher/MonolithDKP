...
function MonDKP:SetOffspecBidding(bool)
	offspecBid = bool

	if core.BiddingWindow then
		core.BiddingWindow.OSItemCheck:SetChecked(bool)
	end
end

function MonDKP:ResetOffspecBidding()
	MonDKP:SetOffspecBidding(not offspecBid)
end


function MonDKP:SetBlindBidding(bool)
	blindBidding = bool

	if core.BiddingWindow then
		core.BiddingWindow.BlindBiddingCheck:SetChecked(bool)
	end
end

function MonDKP:ResetBlindBidding()
	MonDKP:SetBlindBidding(not blindBidding)
end
	...
	if core.IsOfficer then
		...
		if MonDKP.UIConfig then MonDKP.UIConfig:SetFrameLevel(2) end
	 	...
	 	if loot then
			...
			-- New function, allows you to preview item looks and link it to chat (not a perfect link)
			MonDKP.util.AddPreviewSupport(core.BiddingWindow.ItemTooltipButton, itemName)
	 	else
	 		UpdateBidWindow()
	 	end
...
local function StartBidding()
	...
	if mode == "Minimum Bid Values" or (mode == "Zero Sum" and MonDKP_DB.modes.ZeroSumBidType == "Minimum Bid") then
		-- Code to make the bid window appear for yourself
		...
		MonDKP.Sync:SendData("MonDKPCommand", "BidInfo,"..core.BiddingWindow.item:GetText()..","..core.BiddingWindow.minBid:GetText()..","..CurrItemIcon..","..core.BiddingWindow.boss:GetText())
		MonDKP:CurrItem_Set(core.BiddingWindow.item:GetText(), core.BiddingWindow.minBid:GetText(), CurrItemIcon, core.BiddingWindow.boss:GetText())
		...
		if not search and core.BiddingWindow.minBid:GetNumber() ~= tonumber(val) then
			--[[print("INSERTING")
			print("Bid amount: "..core.BiddingWindow.minBid:GetNumber())
			print(tonumber(val))]]
		...
		MonDKP.Sync:SendData("MonDKPCommand", "BidInfo,"..core.BiddingWindow.item:GetText()..","..core.BiddingWindow.cost:GetText()..perc..","..CurrItemIcon..","..core.BiddingWindow.boss:GetText())
		MonDKP:BidInterface_Toggle()
		MonDKP:CurrItem_Set(core.BiddingWindow.item:GetText(), core.BiddingWindow.cost:GetText()..perc, CurrItemIcon, core.BiddingWindow.boss:GetText())
	end
	...
	if CurrItemForBid then
		...
		local suffix = blindBidding and " [Hiding bids for "..UnitName("player").."]" or ""
		if mode == "Minimum Bid Values" or (mode == "Zero Sum" and MonDKP_DB.modes.ZeroSumBidType == "Minimum Bid") then
			SendChatMessage(L["TAKINGBIDSON"].." "..core.BiddingWindow.item:GetText().." ("..core.BiddingWindow.minBid:GetText().." "..L["DKPMINBID"]..")"..suffix, "RAID_WARNING")
		...
		elseif mode == "Static Item Values" or (mode == "Zero Sum" and MonDKP_DB.modes.ZeroSumBidType == "Static") then
			SendChatMessage(L["TAKINGBIDSON"].." "..core.BiddingWindow.item:GetText().." ("..core.BiddingWindow.cost:GetText()..perc..")"..suffix, "RAID_WARNING")
		...
		elseif mode == "Roll Based Bidding" then
			SendChatMessage(L["ROLLFOR"].." "..core.BiddingWindow.item:GetText().." ("..core.BiddingWindow.cost:GetText()..perc..")"..suffix, "RAID_WARNING")
...
function MonDKP:StartBidTimer(seconds, title, itemIcon)
	...
	MonDKP.BidTimer:SetScript("OnUpdate", function(self, elapsed)
		...
		if (timer >= duration) then
			...
			ActionButton_HideOverlayGlow(core.BidInterface.LootTableButton)
...
function MonDKP:CreateTimer()
	...
	local f = CreateFrame("StatusBar", nil, UIParent, "BackdropTemplate")
	f:SetSize(300, 25)
...
local function BidRow_OnClick(self)
	local newPlayer = strsub(self.Strings[1]:GetText(), 1, strfind(self.Strings[1]:GetText(), " ")-1)
	local newBid = tonumber(self.Strings[2]:GetText())
	if (SelectedBidder.player == newPlayer) and (SelectedBidder.bid == newBid) then
		for i=1, numrows do
			core.BiddingWindow.bidTable.Rows[i]:SetNormalTexture("Interface\\COMMON\\talent-blue-glow")
		...
		if MonDKP_DB.modes.costvalue == "Percent" then
			SelectedBidder = {
				player = newPlayer,
				dkp = tonumber(self.Strings[3]:GetText())
			}
		else
			SelectedBidder = {
				player = newPlayer, 
			...
			core.BiddingWindow.cost:SetText(newBid)
		end
	end
end
...
function BidScrollFrame_Update(override)
	-- Hide this until the last 1s
	local bidDuration = tonumber(core.BiddingWindow.bidTimer:GetText())
	if bidDuration then
		local timeLeft = (bidDuration - Timer)
		if blindBidding and (timeLeft > 1) and (not override) then
			C_Timer.After(timeLeft, function() BidScrollFrame_Update(override) end)
			return
		end
	end
	--if ((Timer > 4) and blindBidding) then print("not ready yet") return end
	...
	-- I disabled this, it's kind of annoying
	-- Disabling this might break percent-based bids, but I don't use those
	--[[if mode == "Minimum Bid Values" or (mode == "Zero Sum" and MonDKP_DB.modes.ZeroSumBidType == "Minimum Bid") then
...
function MonDKP:CreateBidWindow()
	local f = CreateFrame("Frame", "MonDKP_BiddingWindow", UIParent, "BackdropTemplate,ShadowOverlaySmallTemplate")
	mode = MonDKP_DB.modes.mode
	...
	tinsert(UISpecialFrames, f:GetName()) -- Sets frame to close on "Escape"
	  ...
	  -- Close Button
	f.closeContainer = CreateFrame("Frame", "MonDKPBiddingWindowCloseButtonContainer", f, "BackdropTemplate")
	...
	if core.IsOfficer then
		...
		f.boss = CreateFrame("EditBox", nil, f, "BackdropTemplate")
		...
		if mode == "Minimum Bid Values" or (mode == "Zero Sum" and MonDKP_DB.modes.ZeroSumBidType == "Minimum Bid") then
			...
			f.minBid = CreateFrame("EditBox", nil, f, "BackdropTemplate")
		...
		f.bidTimer = CreateFrame("EditBox", nil, f, "BackdropTemplate")
		...
		f.OSItemCheck = CreateFrame("CheckButton", nil, f, "UICheckButtonTemplate")
		f.OSItemCheck:SetChecked(offspecBid)
		f.OSItemCheck:SetScale(0.6)
		f.OSItemCheck.text:SetText("  |cff5151deOff spec bid|r")
		f.OSItemCheck.text:SetScale(1.5)
		f.OSItemCheck.text:SetFontObject("MonDKPSmallLeft")
		f.OSItemCheck:SetPoint("BOTTOM", f.StartBidding, "TOP", 0, 10)
		f.OSItemCheck:SetPoint("LEFT", f.StartBidding, "LEFT", 0, 0)
		f.OSItemCheck:SetScript("OnClick", function(self)
			offspecBid = (not offspecBid)

			if core.BiddingWindow and core.BiddingWindow.minBid and CurrItemForBid then
				core.BiddingWindow.minBid:SetText(MonDKP:GetMinBid(CurrItemForBid))
			end
		end)
		...
		f.BlindBiddingCheck = CreateFrame("CheckButton", nil, f, "UICheckButtonTemplate")
		f.BlindBiddingCheck:SetChecked(offspecBid)
		f.BlindBiddingCheck:SetScale(0.6)
		f.BlindBiddingCheck.text:SetText("  |cff5151deHide bidding|r")
		f.BlindBiddingCheck.text:SetScale(1.5)
		f.BlindBiddingCheck.text:SetFontObject("MonDKPSmallLeft")
		f.BlindBiddingCheck:SetPoint("BOTTOM", f.OSItemCheck, "TOP", 0, 10)
		f.BlindBiddingCheck:SetPoint("LEFT", f.OSItemCheck, "LEFT", 0, 0)
		f.BlindBiddingCheck:SetScript("OnClick", function(self)
			blindBidding = (not blindBidding)
		end)
		...
		-- Bid Table
		--------------------------------------------------
		f.bidTable = CreateFrame("ScrollFrame", "MonDKP_BidWindowTable", f, "BackdropTemplate,FauxScrollFrameTemplate")
		...
		f.BidTable_Headers = CreateFrame("Frame", "MonDKPDKPTableHeaders", f, "BackdropTemplate")
		...
		f.StartBidding:SetScript("OnClick", function ()	-- confirmation dialog to remove user(s)
			if SelectedBidder["player"] then
				...
				MonDKP:AwardConfirm(SelectedBidder["player"], SelectedBidder["bid"], f.boss:GetText(), MonDKP_DB.bossargs.CurrentRaidZone, CurrItemForBid)				
			else
