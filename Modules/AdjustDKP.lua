...
local function createPopup(text)
	StaticPopupDialogs["LOOT_SELECT_ERROR"] = {
		text = text,
		button1 = L["OK"],
		timeout = 5,
		whileDead = true,
	...
	StaticPopup_Show("LOOT_SELECT_ERROR")
end
...
local function RaidTimerPopout_Create()
	if not MonDKP.RaidTimerPopout then
		MonDKP.RaidTimerPopout = CreateFrame("Frame", "MonDKP_RaidTimerPopout", UIParent, "BackdropTemplate,ShadowOverlaySmallTemplate")
	    ...
	    -- Popout Close Button
	    MonDKP.RaidTimerPopout.closeContainer = CreateFrame("Frame", "MonDKPChangeLogClose", MonDKP.RaidTimerPopout, "BackdropTemplate")
...
function MonDKP:AdjustDKPTab_Create()
	...
	MonDKP.ConfigTab2.reasonDropDown = LibDD:Create_UIDropDownMenu("MonDKPConfigReasonDropDown", MonDKP.ConfigTab2)
	MonDKP.ConfigTab2.reasonDropDown:SetPoint("TOPLEFT", MonDKP.ConfigTab2.description, "BOTTOMLEFT", -23, -60)
	LibDD:UIDropDownMenu_SetWidth(MonDKP.ConfigTab2.reasonDropDown, 150)
	LibDD:UIDropDownMenu_SetText(MonDKP.ConfigTab2.reasonDropDown, L["SELECTREASON"])
	MonDKP.ConfigTab2.reasonDropDown.lastReason = L["SELECTREASON"]

	-- Create and bind the initialization function to the dropdown menu
	LibDD:UIDropDownMenu_Initialize(MonDKP.ConfigTab2.reasonDropDown, function(self, level, menuList)
	...
	-- Could add parital matching to this in the future
	local function getRaidValue(instance)
		if presetOptions[instance] then
			local killBonus = presetOptions[instance].BossKillBonus
			if killBonus then
				return killBonus
			end
		end

		return MonDKP_DB.DKPBonus.BossKillBonus
	end
	...
	function MonDKP.ConfigTab2.reasonDropDown:SetValue(newValue)
		...
		MonDKP.ConfigTab2.reasonDropDown.lastReason = newValue

		LibDD:UIDropDownMenu_SetText(MonDKP.ConfigTab2.reasonDropDown, curReason)

		if MonDKP.ConfigTab2.addDKP then
			MonDKP.ConfigTab2.addDKP.CurrentRaid = nil
		end

		if (curReason == L["ONTIMEBONUS"]) then
			MonDKP.ConfigTab2.addDKP:SetNumber(MonDKP_DB.DKPBonus.OnTimeBonus)
			MonDKP.ConfigTab2.BossKilledDropdown:Hide()
		elseif (curReason == L["BOSSKILLBONUS"]) then
			local raidZone = core.CurrentRaidZone
			MonDKP.ConfigTab2.addDKP:SetNumber(getRaidValue(raidZone))
			MonDKP.ConfigTab2.addDKP.CurrentRaid = raidZone
			MonDKP.ConfigTab2.BossKilledDropdown:Show()
			LibDD:UIDropDownMenu_SetText(MonDKP.ConfigTab2.BossKilledDropdown, core.CurrentRaidZone..": "..core.LastKilledBoss)
		elseif (curReason == L["RAIDCOMPLETIONBONUS"]) then
			MonDKP.ConfigTab2.addDKP:SetNumber(MonDKP_DB.DKPBonus.CompletionBonus)
			MonDKP.ConfigTab2.BossKilledDropdown:Hide()
		elseif (curReason == L["NEWBOSSKILLBONUS"]) then
			MonDKP.ConfigTab2.addDKP:SetNumber(MonDKP_DB.DKPBonus.NewBossKillBonus)
			MonDKP.ConfigTab2.BossKilledDropdown:Show()
			LibDD:UIDropDownMenu_SetText(MonDKP.ConfigTab2.BossKilledDropdown, core.CurrentRaidZone..": "..core.LastKilledBoss)
		elseif (curReason == L["UNEXCUSEDABSENCE"]) then
			MonDKP.ConfigTab2.addDKP:SetNumber(MonDKP_DB.DKPBonus.UnexcusedAbsence)
			MonDKP.ConfigTab2.BossKilledDropdown:Hide()
		else MonDKP.ConfigTab2.addDKP:SetText("")
			MonDKP.ConfigTab2.BossKilledDropdown:Hide()
		end
	...
	MonDKP.ConfigTab2.otherReason = CreateFrame("EditBox", nil, MonDKP.ConfigTab2, "BackdropTemplate")
	...
	MonDKP.ConfigTab2.BossKilledDropdown = LibDD:Create_UIDropDownMenu("MonDKPBossKilledDropdown", MonDKP.ConfigTab2)
	MonDKP.ConfigTab2.BossKilledDropdown:SetPoint("TOPLEFT", MonDKP.ConfigTab2.reasonDropDown, "BOTTOMLEFT", 0, 2)
	MonDKP.ConfigTab2.BossKilledDropdown:Hide()
	LibDD:UIDropDownMenu_SetWidth(MonDKP.ConfigTab2.BossKilledDropdown, 210)
	LibDD:UIDropDownMenu_SetText(MonDKP.ConfigTab2.BossKilledDropdown, L["SELECTBOSS"])

	LibDD:UIDropDownMenu_Initialize(MonDKP.ConfigTab2.BossKilledDropdown, function(self, level, menuList)
	...
	function MonDKP.ConfigTab2.BossKilledDropdown:SetValue(newValue)
		...
		MonDKP.ConfigTab2.addDKP:SetNumber(getRaidValue(core.CurrentRaidZone))
	...
	-- Add DKP Edit Box
	MonDKP.ConfigTab2.addDKP = CreateFrame("EditBox", nil, MonDKP.ConfigTab2, "BackdropTemplate")
	...
	--[[MonDKP.ConfigTab2.giveLootButton = self:CreateButton("TOPLEFT", MonDKP.ConfigTab2.adjustButton, "BOTTOMLEFT", -1, -15, "Award Loot")
	MonDKP.ConfigTab2.giveLootButton:SetSize(90,25)
	MonDKP.ConfigTab2.giveLootButton:SetScript("OnClick", function()
		if (#core.SelectedData > 1) then
			createPopup("You can only select 1 player at a time.")
		elseif (#core.SelectedData == 1) then
			MonDKP:CreateLootWindow()
		else
			createPopup("No player selected.")
		end
	end)
	MonDKP.ConfigTab2.giveLootButton:SetScript("OnEnter", function(self)
		GameTooltip:SetOwner(self, "ANCHOR_RIGHT")
		GameTooltip:SetText(L["ADJUSTDKP"], 0.25, 0.75, 0.90, 1, true)
		GameTooltip:AddLine(L["ADJUSTDKPTTDESC"], 1.0, 1.0, 1.0, true)
		GameTooltip:AddLine(L["ADJUSTDKPTTWARN"], 1.0, 0, 0, true)
	...
	MonDKP.ConfigTab2.giveLootButton:SetScript("OnLeave", function(self)
		GameTooltip:Hide()
	end)]]

	-- weekly decay Editbox
	MonDKP.ConfigTab2.decayDKP = CreateFrame("EditBox", nil, MonDKP.ConfigTab2, "BackdropTemplate")
	...
	MonDKP.ConfigTab2.RaidTimerContainer = CreateFrame("Frame", nil, MonDKP.ConfigTab2, "BackdropTemplate")
	...
	MonDKP.ConfigTab2.RaidTimerContainer:SetBackdropBorderColor(0.12, 0.12, 0.34, 1)
			...
			if not core.RaidInProgress then				
				if MonDKP_DB.DKPBonus.GiveRaidStart and self:GetText() ~= L["CONTINUERAID"] then
					StaticPopupDialogs["START_RAID_BONUS"] = {
						...
						button2 = L["NO"],
						OnAccept = function()	
							local raidLeader = MonDKP:GetRaidLeader()
							if raidLeader and (not MonDKP:IsValidOfficer(raidLeader)) then
								MonDKP:Print("The raid leader is not a DKP officer. Add them or select a new raid leader to initialize the raid.")
								return
							end
		...
		MonDKP.ConfigTab2.RaidTimerContainer.interval = CreateFrame("EditBox", nil, MonDKP.ConfigTab2.RaidTimerContainer, "BackdropTemplate")
		...
		MonDKP.ConfigTab2.RaidTimerContainer.bonusvalue = CreateFrame("EditBox", nil, MonDKP.ConfigTab2.RaidTimerContainer, "BackdropTemplate")
