...
function MonDKP.Sync:OnEnable()
	...
	MonDKP.Sync:RegisterComm("MonDKPDelSync", MonDKP.Sync:OnCommReceived())			-- broadcasts deleted DKP history entries
	...
	if prefix then
		...
		if prefix == "MonDKPQuery" then
			...
			--if message ~= "start" then
				...
				if MonDKP:ValidateSender(off1) and MonDKP:ValidateSender(off2) and tonumber(date1) > MonDKP_DB.defaults.installed210 and tonumber(date2) > MonDKP_DB.defaults.installed210 then  -- send only if posting officer validates and the post was made after 2.1s installation
					...
					local off2, myDate2 
					if search1 then
						off1, myDate1 = strsplit("-", MonDKP_DKPHistory[1].index)
						off2, myDate2 = strsplit("-", MonDKP_Loot[1].index)
					end

					--if not search1 then
					if not search1 or (myDate1 and (tonumber(date1) > tonumber(myDate1))) then
						MonDKP_DKPHistory.seed = DKP
					end
					--if not search2 then
					if not search2 or (myDate2 and (tonumber(date2) > tonumber(myDate2))) then
						MonDKP_Loot.seed = Loot
					end
				end
			end
		...
		elseif prefix == "MonDKPBuild" and sender ~= UnitName("player") then
			...
			if LastVerCheck > 900 then   					-- limits the Out of Date message from firing more than every 15 minutes 
				if tonumber(message) > core.BuildNumber then
					...
					--[[print("Expected version: "..tonumber(message))
					print("Our version: "..core.BuildNumber)
					print(sender)]]
				end
			end
		...
		if MonDKP:ValidateSender(sender) then		-- validates sender as an officer. fail-safe to prevent addon alterations to manipulate DKP table
			...
			elseif (prefix == "MonDKPCommand") then
				-- What kind of idiot designed this to have commas separating the arguments!
				-- When BC comes out, rewrite this horse shit
 				--local command, arg1, arg2, arg3 = strsplit(",", message)

 				local itemName = string.match(message, "%[.*]")
 				local argTab = {strsplit(",", message)}
 				-- We have a naughty item with a comma in the name (!)
 				if itemName and string.find(itemName, ",", nil, true) then
	 				local itemLink, itemIndex
	 				for index, str in pairs(argTab) do
	 					-- This is the item with the link
	 					if string.find(str, "[", nil, true) then
	 						local _, tempLink = GetItemInfo(str)
	 						itemLink = tempLink
	 						itemIndex = index
	 						break
	 					end
	 				...
	 				if itemIndex then
	 					-- Replace the first half of the name and delete the 2nd
	 					argTab[itemIndex] = itemLink
	 					table.remove(argTab, itemIndex + 1)
	 				end
	 			end

	 			local command = argTab[1]
				...
				if sender ~= UnitName("player") then
					if command == "StartTimer" then
						MonDKP:StartTimer(args[1], args[2])
					elseif command == "StartBidTimer" then
						MonDKP:StartBidTimer(args[1], args[2], args[3])
						core.BiddingInProgress = true;
						if strfind(args[1], "{") then
							MonDKP:Print("Bid timer extended by "..tonumber(strsub(args[1], strfind(args[1], "{")+1)).." seconds.")
						end
					...
					elseif command == "BidInfo" then
						...
						MonDKP:CurrItem_Set(args[1], args[2], args[3], args[4])	-- populates bid window
					end
				end
				...
				prefix == "MonDKPAllTabs" or prefix == "MonDKPBidShare" or prefix == "MonDKPMerge" then
					...
					if success then
						if prefix == "MonDKPAllTabs" then   -- receives full table broadcast
							...
							-- New message
							MonDKP:Print(sender.." finished the full table broacast.")
						...
						elseif prefix == "MonDKPMerge" then
							...
							-- New message
							MonDKP:Print(sender.." finished the merge broadcast.")
						...
						elseif prefix == "MonDKPMinBid" then
							if core.IsOfficer then
								...
								MonDKP_DB.BidInstances = deserialized[3]
							end
						...
						elseif prefix == "MonDKPBossLoot" then
							...
							MonDKP:UpdateLootTable(deserialized)
						end
					else
...
function MonDKP.Sync:SendData(prefix, data, target)
	...
	if IsInGuild() and core.IsOfficer then
		...
		if (prefix == "MonDKPAllTabs") or (prefix == "MonDKPMerge") then
			-- Sending data related to table updates
			if target then
