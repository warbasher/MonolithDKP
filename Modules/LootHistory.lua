...
-- Custom defaults for the new search code
local curSearchFilter = "Item"
local curFilterText = ""
local b_WasSearching = false

local searchOptions = {
	"Item", "Player", "Class", "Boss", "Zone"
}
...
-- sorts the Loot History Table by date
function MonDKP:SortLootTable()
	table.sort(MonDKP_Loot, function(a, b)
		return a["date"] > b["date"]
	end)
end

-- sorts player list alphabetically
local function SortPlayerTable(arg)
...
local function UpdateSearchFilter(filterText)
	-- If we went from searching -> not searching, force an update, otherwise only update while we're searching
	curFilterText = filterText
	local isSearching = string.len(filterText) >= 3
	if (b_WasSearching ~= isSearching) or isSearching then
		MonDKP:LootHistory_Update(curfilterName, curSearchFilter)
	end

	b_WasSearching = isSearching
end
...
function CreateSortBox()
	...
	if not sortDropdown then
		sortDropdown = LibDD:Create_UIDropDownMenu("MonDKPConfigFilterNameDropDown", MonDKP.ConfigTab5)
	end

	-- Add the search button
	if (not searchBox) then
		searchBox = CreateFrame("EditBox", "LootHistory", MonDKP.ConfigTab5, "BackdropTemplate")
		searchBox:SetSize(140, 24)
		searchBox:SetPoint("TOPRIGHT", MonDKP.ConfigTab5, "TOPRIGHT", -175, -13)
		...
		searchBox:SetBackdropBorderColor(1,1,1,0.6)
		searchBox:SetMaxLetters(50)
		searchBox:SetTextColor(0.4, 0.4, 0.4, 1)
		searchBox:SetFontObject("MonDKPNormalLeft")
		searchBox:SetTextInsets(10, 10, 5, 5)
		searchBox:SetText(L["SEARCH"])
		searchBox:SetScript("OnKeyUp", function(self)    -- clears text and focus on esc
			if (searchBox:GetText():match("[%^%$%(%)%%%.%[%]%*%+%-%?]")) then
				searchBox:SetText(string.gsub(searchBox:GetText(), "[%^%$%(%)%%%.%[%]%*%+%-%?]", ""))
			else
				--MonDKP:FilterDKPTable(core.currentSort, "reset")
			end

			UpdateSearchFilter(searchBox:GetText())
		end)
		searchBox:SetScript("OnEscapePressed", function(self)    -- clears text and focus on esc
			self:SetText(L["SEARCH"])
			self:SetTextColor(0.3, 0.3, 0.3, 1)
			self:ClearFocus()
			UpdateSearchFilter("")
		end)
		searchBox:SetScript("OnEnterPressed", function(self)    -- clears text and focus on enter
			self:ClearFocus()
		end)
		searchBox:SetScript("OnTabPressed", function(self)    -- clears text and focus on tab
			self:ClearFocus()
		end)
		...
		searchBox:SetScript("OnEnter", function(self)
			...
			GameTooltip:AddLine("Filters the item list based on item name. Requires 3 characters before searching starts.", 1.0, 1.0, 1.0, true)
			GameTooltip:Show()
		end)
		...
		searchDropdown = LibDD:Create_UIDropDownMenu("MonDKPSearchFilterNameDropDown", MonDKP.ConfigTab5)
		LibDD:UIDropDownMenu_Initialize(searchDropdown, function(self, level, menuList)
			for i=1, #searchOptions do
				local filterName = LibDD:UIDropDownMenu_CreateInfo()
				filterName.func = self.FilterSetValue
				filterName.text = searchOptions[i]
				filterName.arg1 = searchOptions[i]
				filterName.checked = (searchOptions[i] == curSearchFilter)
				filterName.isNotRadio = true
				LibDD:UIDropDownMenu_AddButton(filterName)
			end
		end)

		searchDropdown:SetPoint("TOPRIGHT", MonDKP.ConfigTab5, "TOPRIGHT", -158, -40)

		-- Depends on the max width of the search text filters
		--UIDropDownMenu_SetWidth(searchDropdown, 125)
		LibDD:UIDropDownMenu_SetWidth(searchDropdown, 70)
		LibDD:UIDropDownMenu_SetText(searchDropdown, curSearchFilter or "Filter Name")

		-- Dropdown Menu Function
		function searchDropdown:FilterSetValue(newValue)
			curSearchFilter = newValue
			LibDD:UIDropDownMenu_SetText(searchDropdown, newValue)

			-- If we dont't have any search text, there's no point in updating
			if (string.len(curFilterText) >= 3) then
				MonDKP:LootHistory_Update(curfilterName, newValue)
			end
	...
	LibDD:UIDropDownMenu_SetWidth(sortDropdown, 125)
	LibDD:UIDropDownMenu_SetText(sortDropdown, curfilterName or "Filter Name")
	...
	-- Dropdown Menu Function
  	function sortDropdown:FilterSetValue(newValue, arg2)
		...
		LibDD:UIDropDownMenu_SetText(sortDropdown, arg2)
		MonDKP:LootHistory_Update(newValue, curSearchFilter)
		LibDD:CloseDropDownMenus()
 	end
end
...
local function PassesSearchText(searchInput, lootTab)
	-- Much more optimized at finding the class
	if (curSearchFilter == "Class") then
		for k, v in pairs(MonDKP_DKPTable) do
			if (v.player == lootTab.player) then
				return strfind(string.upper(v.class), string.upper(searchInput))
			end
		end

		return false
	elseif (curSearchFilter == "Item") then
		return strfind(string.upper(lootTab.loot), string.upper(searchInput))
	elseif (curSearchFilter == "Player") then
		return strfind(string.upper(lootTab.player), string.upper(searchInput))
	elseif (curSearchFilter == "Boss") then
		return strfind(string.upper(lootTab.boss), string.upper(searchInput))
	elseif (curSearchFilter == "Zone") then
		return strfind(string.upper(lootTab.zone), string.upper(searchInput))
	end
end
...
function MonDKP:LootHistory_Update(filter, searchFilter)				-- if "filter" is included in call, runs set assigned for when a filter is selected in dropdown.
	...
	-- Adding a filter and removing it glitches out the 'load more' button
	-- Only 'No Filter' should have the add more button
	if lastFilter and (lastFilter ~= filter) and (filter == L["NOFILTER"]) then
		if MonDKP.ConfigTab5.LoadHistory then
			MonDKP.ConfigTab5.LoadHistory:Hide()
			MonDKP.ConfigTab5.LoadHistory = nil
		end
	end
	...
	local hasSearchText = (string.len(curFilterText) >= 3)
	...
	if filter and filter ~= L["NOFILTER"] and filter ~= L["DELETEDENTRY"] then
		for i=1, #MonDKP_Loot do
			if not MonDKP_Loot[i].deletes and not MonDKP_Loot[i].deletedby and not MonDKP_Loot[i].hidden and MonDKP_Loot[i].player == filter then
				if (hasSearchText and PassesSearchText(curFilterText, MonDKP_Loot[i])) or (not hasSearchText) then
	...
	-- Custom code for the search bar
	...
	if core.IsOfficer == true then
		...
		if MonDKP.ConfigTab6.inst then
