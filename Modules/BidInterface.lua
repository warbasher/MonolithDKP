...
local CurrItemForBid, CurrItemIcon, curBoss
...
local function UpdateBidderWindow()
	...
	-- Should NEVER rely on the client to get this info, use the one provided (NOT GetItemInfo)
	local itemLink = CurrItemForBid
	local itemIcon = CurrItemIcon
	local boss = curBoss
	...
	core.BidInterface.LootTableIcon:SetTexture(tonumber(itemIcon))
	core.BidInterface.LootTableIcon:Show()
	core.BidInterface.LootTableButton:Show()





	MonDKP.util.AddPreviewSupport(core.BidInterface.LootTableButton, itemLink)
	ActionButton_ShowOverlayGlow(core.BidInterface.LootTableButton)
	...
	core.BidInterface.item:SetText(itemLink)
	core.BidInterface.Boss:SetText(boss or MonDKP_DB.bossargs.LastKilledBoss)

	core.BidInterface.ItemTooltipButton:SetSize(core.BidInterface.item:GetStringWidth() + 10, core.BiddingWindow.item:GetHeight())
	...
	for i=1, 2 do
		...
		local curElement = ((i == 1 and core.BidInterface.LootTableButton) or core.BidInterface.ItemTooltipButton)
		curElement:SetScript("OnEnter", function(self)
		...
		curElement:SetScript("OnLeave", function(self)
			GameTooltip:Hide()
		end)
		MonDKP.util.AddPreviewSupport(curElement, itemLink)
	end
end
...
function MonDKP:BidInterface_Toggle()
	...
	-- Update our DKP display
	if core.BidInterface.CurDKP then
		local playerDKP = MonDKP:Table_Search(MonDKP_DKPTable, UnitName("player"), "player")
		if playerDKP then
			f.CurDKP:SetText(MonDKP_DKPTable[playerDKP[1][1]].dkp)
		end
	end
...
function MonDKP:CurrItem_Set(item, value, icon, boss)
	CurrItemForBid = item
	CurrItemIcon = icon
	curBoss = boss
...
function MonDKP:BidInterface_Create()
	local f = CreateFrame("Frame", "MonDKP_BidderWindow", UIParent, "BackdropTemplate,ShadowOverlaySmallTemplate")
	...
	tinsert(UISpecialFrames, f:GetName()) -- Sets frame to close on "Escape"
	  ...
	  -- Close Button
	f.closeContainer = CreateFrame("Frame", "MonDKPBidderWindowCloseButtonContainer", f, "BackdropTemplate")
	...
	f.LootTableIcon = f:CreateTexture(nil, "OVERLAY", nil)
	f.LootTableIcon:SetColorTexture(0, 0, 0, 1)
	f.LootTableIcon:SetSize(35, 35)
	f.LootTableIcon:Hide()
	f.LootTableIcon:SetPoint("LEFT", f.lootContainer, "LEFT", 0, 0)

	f.LootTableButton = CreateFrame("Button", "MonDKPBidderLootTableButton", f)
	f.LootTableButton:SetPoint("TOPLEFT", f.LootTableIcon, "TOPLEFT", 0, 0)
	f.LootTableButton:SetSize(35, 35)
	f.LootTableButton:Hide()
	...
	f.ItemTooltipButton = CreateFrame("Button", nil, f)
	f.ItemTooltipButton:SetPoint("TOPLEFT", f.item, "TOPLEFT", 0, 0)
	...
	-- New DKP display in the corner
	f.DKPHeader = f:CreateFontString(nil, "OVERLAY")
	f.DKPHeader:SetFontObject("MonDKPLargeRight")
	f.DKPHeader:SetScale(0.7)
	f.DKPHeader:SetText("Current DKP:")
	f.DKPHeader:SetPoint("TOPLEFT", f, "TOPLEFT", 40, -50)
	--f.DKPHeader:SetPoint("TOPRIGHT", f.BidHeader, "BOTTOMRIGHT", 0, -16)

	local playerDKP = MonDKP:Table_Search(MonDKP_DKPTable, UnitName("player"), "player")
	if playerDKP then
		f.CurDKP = f:CreateFontString(nil, "OVERLAY")
		f.CurDKP:SetFontObject("MonDKPNormal")
		f.CurDKP:SetTextColor(1, 1, 1, 1)
		f.CurDKP:SetText(MonDKP_DKPTable[playerDKP[1][1]].dkp)
		f.CurDKP:SetPoint("CENTER", f.DKPHeader, "BOTTOM", 0, -8)
	end



	f.Bid = CreateFrame("EditBox", nil, f, "BackdropTemplate")
	...
	-- Bid Table
	--------------------------------------------------
    f.bidTable = CreateFrame("ScrollFrame", "MonDKP_BiderWindowTable", f, "BackdropTemplate,FauxScrollFrameTemplate")
	...
	f.BidTable_Headers = CreateFrame("Frame", "MonDKPBidderTableHeaders", f.bidTable, "BackdropTemplate")
