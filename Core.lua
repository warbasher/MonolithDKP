...
local tc_colors = {
	...
	["Shaman"] = { r = 0, g = 0.44, b = 0.98, hex = "0070DD" }
}

local tc_classes = {}

core.faction = UnitFactionGroup("player")
tc_classes = { "Druid", "Hunter", "Mage", "Paladin", "Priest", "Rogue", "Warlock", "Warrior", "Shaman" }
...
core.EncounterList = {      -- Event IDs must be in the exact same order as core.BossList declared in localization files
	...
	-- This should REALLY check the NPC name and NOT the encounter ID for the auto-DKP
	KARA = {
		652, 653, 654,
		655, 656, 657,
		658, 659, 660,
	...
	--Test = {"Defias Insurgent", "Defias Convict"},
}

core.MonDKPUI = {}        -- global storing entire Configuration UI to hide/show UI
core.MonVersion = "v2.2.2"
core.BuildNumber = 20202
core.Changelog = "- Added item link (shift + mouse click) and item preview support (ctrl + mouse click) to the bid interface\n- Added item preview support to loot history\n- Properly aligned item tooltips for the bid interface\n- The bid interface now displays tooltips over the item name as well (was previously just the icon)"
--[[core.Changelog = {
	{Version = "v2.2.1", Description = "- Fixed the 'Current DKP' in the bid window not updating\n- Removed the 'Award Item' button in the 'Adjust DKP' tab"},
	{Version = "v2.2.0", Description = "- Added LibUIDropDownMenu to fix taint issues caused by the default dropdown\n- Added preliminary support for phase 2 raids\n- Added a shaman filter and better aligned class icons"},
}]]
...
function MonDKP:GetRaidLeader()
	if IsInRaid() then
		for i=1, 40 do
			local tempName, tempRank = GetRaidRosterInfo(i)
			if tempRank == 2 then
				return tempName
			end
		end
...
function MonDKP:IsValidOfficer(sender)								-- returns true if "sender" has permission to write officer notes. false if not or not found.
...
function MonDKP:PurgeDKPHistory()     -- purges old entries and stores relevant data in each users MonDKP_Archive entry (dkp, lifetime spent, and lifetime gained) 
	...
	if #MonDKP_DKPHistory > limit then
		while #MonDKP_DKPHistory > limit do
			...
			-- If you somehow manage to do this, bravo
			if (#path.players ~= 0) then
...
function MonDKP:CreateContainer(parent, name, header)
	local f = CreateFrame("Frame", "MonDKP"..name, parent, "BackdropTemplate")
	f:SetBackdrop( {
	...
	f.header = CreateFrame("Frame", "MonDKP"..name.."Header", f, "BackdropTemplate")
